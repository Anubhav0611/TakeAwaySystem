...
def Signup(request):
    ...
    if request.method == 'POST':
        ...
        answers = user.filter(username=u)
        if answers:
            unique = True
            d = {'unique':unique}
...
def Customer_Detail(request):
    ...
    if request.method == 'POST':
        ...
        con = request.POST['contact']
        i = request.FILES['img']
        user = User.objects.create_user(username=u, email=e,  first_name=f,last_name=l)
        Profile.objects.create(user=user,  city=c,  contact=con,image=i)
        error = True
    d = {'error':error}
...
def Add_Cart_admin(request,pid):
    if request.method=="POST":
        user = User.objects.latest('id').id
        profile = Profile.objects.get(user=user)
        product = Product.objects.get(id=pid)
        Cart.objects.create(profile=profile, product=product)
        return redirect('cart_admin')
        # return render(request, 'about.html')
...
def Booking_order(request, pid,abc):
    if not request.user.is_authenticated:
        return redirect('login')
    ...
    if request.method == "POST":
        ...
        p1=request.POST['payment']
        user = User.objects.get(username=c)
        profile = Profile.objects.get(user=user)
        status = Status.objects.get(name="pending")
        book1 = Booking.objects.create(profile=profile, book_date=d,booking_id=b,total=t,quantity=num1,status=status,payment=p1)
        cart2 = Cart.objects.filter(profile=profile).all()
        cart2.delete()
        return redirect('payment',book1.total)
    d = {'data': data, 'data1': data1, 'book_id': pid,'username':user1,'email':email1, 'date1': date1,'total':total,'num1':num1,'paymenttype':paymenttype}
    return render(request, 'booking.html', d)

def Booking_order_admin(request, pid,abc):
    if not request.user.is_authenticated:
        return redirect('login')
    data1=User.objects.latest('id').username
    data = Profile.objects.filter(user=User.objects.latest('id').id).first()
    cart = Cart.objects.filter(profile=data).all()
    total = 0
    num1=0
    for i in cart:
        total+=i.product.price
    user1 = data1
    email1=User.objects.latest('id').email
    li = pid.split('.')
    li2 = []
    for j in li:
        if 'admin' != j:
    ...
    if request.method == "POST":
        ...
        cart2.delete()
        return redirect('payment',book1.total)
    d = {'data': data, 'data1': data1, 'book_id': book_id,'username':user1,'email':email1, 'date1': date1,'total':total,'num1':num1,'paymenttype':paymenttype}
    return render(request, 'booking.html', d)
...
def booking_detail(request,pid,bid):
    ...
    if request.user.is_staff:
        user = User.objects.latest('id').id
        user1 = User.objects.latest('id').username +"_admin"
    profile = Profile.objects.get(user=user)
    cart =  Cart.objects.filter(profile=profile).all()
...
def search(request):
    ...
    cat = "All Product"
    pro1 = Product.objects.filter(name__icontains=q)
